...
class HomeViewPagerFragment : Fragment() {
    ...
    private fun getTabIcon(position: Int): Int {
        return when (position) {
            PLANT_LIST_PAGE_INDEX -> R.drawable.ic_list
            MY_GARDEN_PAGE_INDEX -> R.drawable.ic_baseline_favorite_border_24
    ...
    private fun getTabTitle(position: Int): String? {
        return when (position) {
            PLANT_LIST_PAGE_INDEX -> getString(R.string.play_list)
            MY_GARDEN_PAGE_INDEX -> getString(R.string.favours)
            else -> null
        }
    }
}